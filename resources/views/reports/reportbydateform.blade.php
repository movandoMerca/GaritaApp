        ...
        <div class="card-header custom-bg ">
            <div class="card-title">
                <h3 class="card-label text-white">
                    ...
                    <small>Búsqueda por fecha</small>
                </h3>
            </div>
        </div>
        <div class="card-body">
            <form action="{{ route('reportbydate.visits') }}" method="POST"  >
                @csrf
            <div class="row">
                <div class="col-md-6">
                ...
                <div class="col-md-6">
                    ...
                    <input type="date" name="to" id="to" class="form-control @error('to') is-invalid @enderror" max="{{ date('Y-m-d') }}" value="{{ date('Y-m-d') }}">
                    @error('to')
                    <div class="alert text-danger">{{ $message }}</div>
                     @enderror
                </div>
